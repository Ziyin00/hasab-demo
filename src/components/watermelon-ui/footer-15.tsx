...
export function Footer15({
  ...
  return (
    <footer className="lp-band-section w-full overflow-hidden rounded-t-4xl font-sans antialiased sm:rounded-t-[2.5rem] md:rounded-t-[3rem]">
      <motion.div
          ...
          >
            <div className="flex items-center gap-2">
              <span className="shrink-0">
                {logoIcon ?? <LogoIcon className="size-8 text-[var(--lp-band-accent)]" />}
              </span>
              <span className="text-lg font-medium uppercase select-none text-[var(--lp-band-fg)]">
                {brandName}
              </span>
            </div>

            <p className="lp-band-muted text-sm leading-[1.6] font-light text-pretty whitespace-pre-line">
              {description}
            </p>
            ...
            <motion.a
              ...
              variants={ctaVariant}
              whileTap={{ scale: 0.96 }}
              className="group mt-1 inline-flex w-fit items-center gap-2.5 rounded-full bg-primary-gradient py-0.5 pr-1 pl-5 transition-opacity duration-200 hover:opacity-90"
            >
              ...
              <span
                ...
                style={{ background: "var(--lp-band-cta-icon-bg)" }}
              >
                <FaArrowRight className="size-4 text-[var(--lp-band-fg)] transition-transform duration-200 group-hover:translate-x-0.5" />
              </span>
            </motion.a>
          </motion.div>
          ...
          >
            {columns.map((col) => (
              <motion.div key={col.title} variants={riseItem} className="">
                <h3 className="text-md leading-none font-normal tracking-wide text-balance text-[var(--lp-band-fg)]">
                  {col.title}
                </h3>
                <motion.ul
                ...
                >
                  {col.links.map((link) => (
                    <motion.li key={link.label} variants={linkItem}>
                      <a
                        href={link.href}
                        className="lp-band-muted inline-block text-sm leading-none font-light transition-colors duration-200 hover:text-[var(--lp-band-fg)]"
                      >
                        {link.label}
                      </a>
                    </motion.li>
      ...
      >
        <svg
          className="h-auto w-full max-w-full translate-y-2 select-none md:translate-y-6"
          viewBox="0 0 800 110"
        ...
        >
          <text
            ...
            textAnchor="middle"
            className="lp-footer-watermark font-sans font-semibold"
            fontSize="140"
          >
            {heroBrandName}
          </text>
